...
忆境 AI 是一个面向高校学生的 AI 电子相册与校园记忆生成器。用户上传一组校园活动照片后，系统会生成结构化图像分析结果、活动故事线、朋友圈九宫格排序方案、四种朋友圈文案，并输出适合比赛展示的电子相册页面。

第一版使用 mock service 验证产品闭环，未接入真实 AI API；页面中的“AI 生成”表述代表演示版产品流程。代码结构已按后续接入真实多模态大模型预留。
...
- 最终展示页支持一键复制朋友圈文案、导出 Markdown 相册分析报告、重新上传照片、返回相册生成页。
...
4. 进入相册生成页，点击“生成相册方案”。
5. 进入最终展示页，复制文案或导出 Markdown 报告。
6. 比赛现场可点击“重新上传照片”重复演示。

## 后续接入真实 AI

当前 mock service 逻辑集中在：
